...
random_seed = sum([ord(x) for x in 'runge']) % 10
np.random.seed(random_seed)


f = lambda x: 1 / (1 + (10 * (x - 0.5)) ** 2)
...
for i, n in enumerate(n_vals):
	...
	for name, approximator in approximators.items():
		...
		mse_errors.to_csv(f'csv_files/runge_mse_{noise_level}.csv')
		max_errors.to_csv(f'csv_files/runge_max_{noise_level}.csv')
